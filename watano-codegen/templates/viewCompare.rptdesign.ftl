...
left join ext_dq_dyhfdj hfdj on (dy.id=hfdj.dyid and hfdj.hfdj_time>='__strYear__-01-01' and hfdj.hfdj_time<='__strYear__-12-31')
left join ext_dq_dydead dead on (dy.id=dead.dyid and dead.dead_time>='__strYear__-01-01' and dead.dead_time<='__strYear__-12-31')
left join ext_dq_dytzdj tzdj on (dy.id=tzdj.dyid and tzdj.tzdj_time>='__strYear__-01-01' and tzdj.tzdj_time<='__strYear__-12-31')
left join ext_dq_dycd cd on (dy.id=cd.dyid and cd.cd_time>='__strYear__-01-01' and cd.cd_time<='__strYear__-12-31')
left join ext_dq_dylzb lzb_in on (dy.id=lzb_in.dyid and (lzb_in.zr_type='3' or lzb_in.lr_type='7') and lzb_in.js_time>='__strYear__-01-01' and lzb_in.js_time<='__strYear__-12-31' and lzb_in.js_orgnizationid in (__orgids__))
left join ext_dq_dylzb lzb_out on (dy.id=lzb_out.dyid and (lzb_out.zc_type='1' or lzb_out.lr_type='5') and lzb_out.zc_time>='__strYear__-01-01' and lzb_out.zc_time<='__strYear__-12-31' and lzb_out.zc_orgnizationid in (__orgids__))
left join ext_dq_dangyuan fzdy on (dy.id=fzdy.id and ((fzdy.rdtime>='__strYear__-01-01' and fzdy.rdTime<='__strYear__-12-31') or fzdy.rdTime is null))
...
<body>
	<@birt.grid cols=6 dataSet='dy' boundDataColumns=[
		...
		<@birt.cell />
		<@birt.cell colSpan=4>
			<@birt.text text='两 年 党 员 数 综 合 比 较' textAlign='center' fontWeight='bold'/>
		</@birt.cell>
		<@birt.cell />

		<@birt.cell><@birt.data col='填报单位'/></@birt.cell>
		<@birt.cell colSpan=4></@birt.cell>
